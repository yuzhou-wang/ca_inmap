...
func Preprocess(p Preprocessor, xo, yo, dx, dy float64) (*CTMData, error) {
	...
	var uDeviation, vDeviation, aOrgPartitioning, aVOC, aSOA, bOrgPartitioning, bVOC, bSOA,
		...
		NOxDryDep, NH3DryDep, VOCDryDep, Kxxyy, PMPartitioning *sparse.DenseArray
	...
	go func() {
		var err error
		PMPartitioning, err = averageone(p.TotalPM25())
		errChan <- err
	}()
	...
	data.AddVariable("PMPartitioning", []string{"z", "y", "x"},
		"Mass fraction of primary PM2.5 that partitioning to PM2.5", "fraction",
		PMPartitioning)
...
func averageone(dataFunc NextData) (*sparse.DenseArray, error) {
	var avgonedata *sparse.DenseArray
	firstData := true
	var n int
	for {
		data, err := dataFunc()
		if err != nil {
			if err == io.EOF {
				return arrayOne(avgonedata, n), nil
			}
			return nil, err
		}
		if firstData {
			avgonedata = sparse.ZerosDense(data.Shape...)
			firstData = false
		}
		avgonedata.AddDense(data)
		n++
	}
}
...
func arrayOne(s *sparse.DenseArray, numTsteps int) *sparse.DenseArray {
	n := float64(numTsteps)
	for i, val := range s.Elements {
		s.Elements[i] = val / n / val * 8760
	}
	return s
}
